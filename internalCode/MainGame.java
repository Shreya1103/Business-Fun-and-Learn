...
public class MainGame {
	...
	private static MainGame gameMaster;
	// The gameboard
	private Board gameBoard;
	// Initial money assigned to each player
	private int initMoney = 30000;
	private int bailAmt = 1000;
	...
	public static MainGame instance() {
		if (gameMaster == null) {
			gameMaster = new MainGame();
		}
		return gameMaster;
	}
	
	private MainGame() {
		chanceCards = new ChanceProducer().build();
		communityCards = new GiftProducer().build();

		cellList = new BoardConstructor().read();
	}
	...
	public void btnPlayClicked() {
		...
		if (plr.isInJail()) {
			if (value.get(3) == 1) {
				...
				plr.setInJail(false);
				gameBoard.btnPayBail.setEnabled(false);
				int new_pos = (pos + value.get(2)) % 36;
				if (pos + value.get(2) >= 36) {
					plr.addMoney(1000);
				}
		...
		if (!plr.isInJail()) {
			gameBoard.btnEndTurn.setEnabled(true);
			Box current = (Box) getCellAtPos(plr.getPosition());
...
//				Subtract the money from the player's chest
	...
	public void btnPickCardClicked() {
		Player plr = getCurrentPlayer();
		Box x = (Box) getCellAtPos(plr.getPosition());
		Card picked = null;
		int card_num = -1;
		if (x.isChance()) {
...
//					But before that, check if he can manage to pay money..
	...
	private void payTax(Player plr) {
		while (true) {
			Box x = (Box) gameMaster.getCellAtPos(plr.getPosition());
			int payable = x.getRent();
			if (plr.getMoney() >= payable) {
				plr.subMoney(payable);
				gameBoard.lblRent.setBounds(343, 250, 172, 20);
	...
	private void payRent(Property p) {
		...
		while (true) {
			if (plr.getMoney() > rent) {
				plr.subMoney(rent);
				oppnt.addMoney(rent);
				gameBoard.lblRent.setBounds(343, 250, 250, 20);
